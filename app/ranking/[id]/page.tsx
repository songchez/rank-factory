    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 text-center">
          <h1 className="font-heading text-4xl md:text-6xl mb-4">{topic.title}</h1>
          <p className="text-lg">최종 랭킹 결과</p>
          <div className="mt-4 flex items-center justify-center gap-4">
            <Link href={`/battle/${topic.id}`}>
              <NeoButton variant="primary">다시 투표하기</NeoButton>
            </Link>
            <NeoButton variant="secondary" onClick={handleShare} className="flex items-center gap-2">
              <Share2 className="w-4 h-4" />
              공유하기
            </NeoButton>
            <NeoButton variant="accent" onClick={() => setIsCommentsOpen(true)} className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-3 gap-4 max-w-4xl mx-auto items-end">
            ...
            <div className="flex flex-col items-center">
              <NeoCard variant="default" className="w-full mb-4 bg-gray-300">
                <div className="relative w-full aspect-square mb-4">
                  <Image src={second.imageUrl || "/placeholder.svg"} alt={second.name} fill className="object-cover" />
                </div>
            ...
            <div className="flex flex-col items-center">
              <NeoCard variant="primary" className="w-full mb-4">
                <div className="relative w-full aspect-square mb-4">
                  <Image src={first.imageUrl || "/placeholder.svg"} alt={first.name} fill className="object-cover" />
            ...
            <div className="flex flex-col items-center">
              <NeoCard variant="default" className="w-full mb-4 bg-orange-300">
                <div className="relative w-full aspect-square mb-4">
                  <Image src={third.imageUrl || "/placeholder.svg"} alt={third.name} fill className="object-cover" />
                </div>
                <div className="text-center">
            ...
            <div className="space-y-4">
              {rest.map((item, index) => (
                <NeoCard key={item.id} className="flex items-center gap-4">
                  <div className="font-heading text-3xl w-12 text-center">{index + 4}</div>
                  ...
                  <div className="text-right">
                    <div className="text-sm font-bold">승률 {Math.round((item.winCount / item.matchCount) * 100)}%</div>
                    <div className="w-32 h-2 bg-muted border border-black mt-2">
                      <div
                        className="h-full bg-primary"
                        style={{ width: `${(item.winCount / item.matchCount) * 100}%` }}
                      />
                    </div>
                  </div>
                </NeoCard>
