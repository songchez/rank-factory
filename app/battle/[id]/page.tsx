...
import { getTopicById, getRandomMatchup, type RankingItem } from "@/lib/mock-data"
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
            ...
            {[...Array(30)].map((_, i) => (
              <div
                ...
                style={{
                  ...
                  top: `${Math.random() * 100}%`,
                  backgroundColor: ["#FFDE59", "#FF5757", "#8C52FF"][Math.floor(Math.random() * 3)],
      ...
      <div className="grid md:grid-cols-2 min-h-screen">
        ...
        >
          <Image src={matchup[0].imageUrl || "/placeholder.svg"} alt={matchup[0].name} fill className="object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
          <div className="absolute bottom-0 left-0 right-0 p-8">
            <h2 className="font-heading text-4xl md:text-6xl text-white mb-2 text-balance">{matchup[0].name}</h2>
            <p className="text-white/80 text-lg">
              ELO {matchup[0].eloScore} • {matchup[0].winCount}승 {matchup[0].lossCount}패
            </p>
          </div>
        ...
        <button
          ...
          style={{ minHeight: "44px" }}
        >
          <Image src={matchup[1].imageUrl || "/placeholder.svg"} alt={matchup[1].name} fill className="object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
          <div className="absolute bottom-0 left-0 right-0 p-8">
            <h2 className="font-heading text-4xl md:text-6xl text-white mb-2 text-balance">{matchup[1].name}</h2>
            <p className="text-white/80 text-lg">
              ELO {matchup[1].eloScore} • {matchup[1].winCount}승 {matchup[1].lossCount}패
            </p>
          </div>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-30">
        <div className="bg-background border-3 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] px-6 py-4 flex items-center gap-4">
          <NeoButton variant="outline" size="sm" onClick={handleSkip} className="flex items-center gap-2">
            <SkipForward className="w-4 h-4" />
            건너뛰기
          </NeoButton>
          <NeoButton variant="primary" size="sm" onClick={handleViewResults} className="flex items-center gap-2">
