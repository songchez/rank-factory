...
export async function generateTopicContent(prompt: string): Promise<GeneratedTopic> {
  ...
  const result = await model.generateContent([systemPrompt, `Prompt: ${prompt}`]);
...
export async function generateImage(prompt: string): Promise<string> {
  ...
  try {
    ...
    if (!part || !part.inlineData) {
      console.warn("No inline data found in response. Response might be text-only or error.");
